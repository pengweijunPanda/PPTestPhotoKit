...
@implementation HGImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.hg_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[HGImageManager manager] fixOrientation:cropedImage];
...
+ (UIImage *)sd_tz_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [HGImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
